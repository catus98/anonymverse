...
app.get('/user/:telegramId', async (req, res) => {
    ...
    log(`User ID: ${telegramId}, Referrer ID: ${referrerId}`);
    ...
    if (!user) {
        ...
        try {
            const response = await axios.get(`${botUrl}/getChat`, { params: { chat_id: telegramId } });
            if (response.data.ok) {
                const chat = response.data.result;
                user.username = `${chat.first_name || ''} ${chat.last_name || ''}`.trim();

                const photoResponse = await axios.get(`${botUrl}/getUserProfilePhotos`, { params: { user_id: telegramId, limit: 1 } });
                if (photoResponse.data.ok && photoResponse.data.result.photos.length > 0) {
                    ...
                    const fileResponse = await axios.get(`${botUrl}/getFile`, { params: { file_id: photoFileId } });
        ...
        if (referrerId) {
            ...
            if (referrer) {
                ...
                log(`Referrer ${referrerId} referred user ${telegramId}.`);
                checkAndCompleteReferralTasks(referrer);
            } else {
                log(`Referrer not found: ${referrerId}`);
            }
        }
        saveUserData(users);
    } else {
        log(`User already exists: ${JSON.stringify(user)}`);
    }
    ...
    tasks.forEach(task => {
        if (task.type === 'referral') {
            ...
            if (!task.completedBy.includes(user.telegramId) && user.referrals.length >= task.requiredReferrals) {
                ...
                tasksUpdated = true;
                log(`User ${user.telegramId} completed task ${task.title} and earned ${task.points} points.`);
            }
        }
    });
...
app.post('/claim/:telegramId', (req, res) => {
    ...
    if (userIndex !== -1) {
        ...
        if (now >= user.nextClaim) {
            ...
            log(`User ${telegramId} claimed points. New points: ${user.points}`);
            ...
            const distributeReferralPoints = (userId, percentage) => {
                ...
                if (referrer) {
                    ...
                    log(`Referrer ${referrer.telegramId} earned ${pointsEarned} points from ${telegramId}.`);
                } else {
                    log(`Referrer not found: ${userId}`);
                }
            };
...
app.post('/complete-task/:telegramId/:taskId', async (req, res) => {
    ...
    if (task.type === 'telegram-check') {
        try {
            ...
            log(`Telegram API response: ${JSON.stringify(response.data)}`);
...
app.post('/invite/:telegramId', async (req, res) => {
    ...
    try {
        ...
        log(`Referral link sent to ${telegramId}.`);
        res.send('Referral link sent');
    } catch (error) {
        log(`Error sending referral link: ${error.message}`);
